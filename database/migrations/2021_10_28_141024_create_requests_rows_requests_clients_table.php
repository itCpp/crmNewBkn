...
class CreateRequestsRowsRequestsClientsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('requests_rows_requests_clients', function (Blueprint $table) {
            $table->foreignId('id_request')->constrained('requests_rows')->onUpdate('cascade')->onDelete('cascade');
            $table->foreignId('id_requests_clients')->constrained('requests_clients')->onUpdate('cascade')->onDelete('cascade');
